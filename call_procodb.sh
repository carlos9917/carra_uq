...
obstype=(1 1 1 1 2 2 4 5 5 5)
codetype=(11 14 21 24 144 144 165 35 35 35)
varno=(1 1 1 1 3 2 1 3 2 7)
for year in 2012;do
  for month in 07; do
    for day in ${days[@]}; do
	    for i in "{!obstype[@]}"; do
            python3 process_odb.py -d "$year/$month/$day" -ot ${obstype[i]} -ct ${codetype[i]} -vn ${varno[i]} >& ./$year/$month/$day/summary.txt
            done
    done #day
   done #month
done #year
